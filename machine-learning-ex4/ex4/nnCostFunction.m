...
% trans y to vectors
temp_y = zeros(max(y), m);
for i = 1:m
  temp_y(y(i), i) = 1;
end

% calc J
temp_K = sum((-temp_y' .* log(h) - (1 - temp_y') .* log(1 - h)), 1);
temp = sum(temp_K);
J = 1 / m * temp;
...
% sigmoidGradient  = ax .* (1 - ax) so if use z2 we need add a2_0 = 1 in the front of list
